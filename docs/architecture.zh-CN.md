...
> 状态：**已采纳（P0 完成）。** 实现活跃（`cmd/`、`internal/`）：P1–P3 核心已落地，部分 P4 能力提前完成。RFC 0001–0005 为规范性默认。
>
...
**已采纳默认**见 [design/rfc/README.zh-CN.md](design/rfc/README.zh-CN.md)（亲和、revision、PostgreSQL、advertised_host、Location 缓存）。

仍开放（不阻塞交付）：

1. 冻结其余 `v1alpha1` 字段名（社区 RFC）。
2. 插件：gRPC webhook vs Wasm（HTTP webhook 已可用）。
3. Redis Cluster 下多租户 hash tag？
4. 完整 `kind: DispatchGroup` 资源加载 vs Trunk `sendOptionsPing` 路径。

推翻已采纳 RFC 前请先开 Discussion。
